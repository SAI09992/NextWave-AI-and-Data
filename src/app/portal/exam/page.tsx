  ...
  if (examState === 'terminated') {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#0a0a0f]">
        <div className="max-w-md w-full p-8 rounded-2xl nexus-glass border border-red-500/50 bg-red-950/20 text-center space-y-6">
          ...
          <NexusButton variant="primary" glow className="w-full bg-red-600 hover:bg-red-500 text-white" onClick={() => router.push('/portal')}>
            RETURN TO PORTAL
          </NexusButton>
        </div>
      </div>
      ...
      <main className="max-w-4xl mx-auto mt-8 px-4 space-y-6">
        {questions.length > 0 && (
          <>
            ...
            {/* Pagination Controls */}
            <div className="flex items-center justify-between p-4 rounded-2xl border border-nexus-border bg-nexus-bg-elevated/50 backdrop-blur-sm">
              <button
                onClick={() => setCurrentQuestionIndex(prev => Math.max(0, prev - 1))}
                disabled={currentQuestionIndex === 0}
                className={`px-6 py-3 rounded-xl font-mono text-sm font-bold transition-colors ${currentQuestionIndex === 0 ? 'opacity-50 cursor-not-allowed bg-nexus-surface text-nexus-text-muted border border-nexus-border' : 'bg-nexus-surface/50 text-nexus-text hover:bg-nexus-surface border border-nexus-border'}`}
              >
                PREVIOUS
              </button>

              <div className="font-mono text-sm text-nexus-text-muted hidden md:block">
      ...
      {showWarningOverlay && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="max-w-md w-full p-8 rounded-3xl bg-red-950/20 border-2 border-red-500 shadow-[0_0_50px_rgba(239,68,68,0.2)] text-center space-y-6">
            ...
            <NexusButton variant="primary" glow className="w-full bg-red-600 hover:bg-red-500 text-white" onClick={resumeFromWarning}>
              ACKNOWLEDGE & RESUME
            </NexusButton>
          </div>
        </div>
