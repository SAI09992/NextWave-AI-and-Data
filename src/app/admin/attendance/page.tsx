  ...
  return (
    <div className="space-y-6 font-mono text-xs max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-nexus-border">
        ...
        {/* Tab Switcher */}
        <div className="inline-flex p-1 rounded-xl bg-nexus-surface border border-nexus-border">
          <button
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-bold transition-all flex items-center gap-2 ${
              activeTab === 'attendance'
                ? 'bg-nexus-primary text-nexus-bg shadow-nexus-glow-sm'
                : 'text-nexus-text-muted hover:text-nexus-text'
            }`}
          >
      ...
      {activeTab === 'snacks' && (
        <div className="space-y-6">
          ...
          <div className="p-4 rounded-2xl nexus-glass border border-amber-500/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            {/* Slot Buttons */}
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-nexus-text-muted text-[11px] font-bold mr-1 flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className={`px-3 py-2 rounded-xl font-bold transition-all flex items-center gap-2 text-xs ${
                      isSelected
                        ...
                        : 'bg-nexus-surface border border-nexus-border text-nexus-text-muted hover:text-nexus-text'
                    }`}
                  >
                    <span>{s.label}</span>
                    <span
                      className={`text-[10px] px-1.5 py-0.2 rounded-md ${
                        isSelected ? 'bg-black/20 text-black' : 'bg-nexus-bg text-amber-400'
                      }`}
                    >
                      {count}
                    </span>
            ...
            <div className="flex items-center gap-2.5 w-full md:w-auto justify-end">
              <button
                ...
                className="p-2 rounded-lg bg-nexus-surface border border-nexus-border text-nexus-text-muted hover:text-amber-400 transition-colors"
                title="Refresh stats"
              >
          ...
          <div className="px-2 flex items-center justify-between text-xs text-nexus-text-dim">
            <div>
              CURRENTLY SCANNING FOR:{' '}
              <strong className="text-amber-400">{currentSlotObj?.name}</strong>
            </div>
            <div>
              SERVED: <strong className="text-nexus-primary">{snacksStats.servedCount}</strong> /{' '}
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <div className="md:col-span-7 p-6 rounded-2xl nexus-glass-glow border border-amber-500/40 space-y-6">
              <CameraQrScanner
              ...
              <div className="pt-4 border-t border-nexus-border/80 space-y-3">
                ...
                <form onSubmit={handleManualSnackSubmit} className="space-y-3">
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl bg-nexus-surface border border-nexus-border text-amber-400 font-bold text-xs focus:outline-none focus:border-amber-400"
                  />
            ...
            <div className="md:col-span-5 p-6 rounded-2xl nexus-glass border border-nexus-border space-y-4 flex flex-col justify-between">
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-nexus-border pb-3">
                  <h3 className="text-sm font-bold text-nexus-text uppercase tracking-wider flex items-center gap-2">
                ...
                {lastScanResult ? (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] px-2 py-0.5 rounded font-bold uppercase bg-black/40 text-nexus-text">
                    ...
                    {/* Cadet Details */}
                    <div className="space-y-2 text-xs border-t border-nexus-border/40 pt-3">
                      <div className="text-base font-bold text-nexus-text">
                        {lastScanResult.participant.name}
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-nexus-text-muted text-[11px]">
                        <div>
                          Reg ID:{' '}
                  ...
                  <div className="p-8 text-center text-nexus-text-dim border border-nexus-border/60 rounded-2xl bg-nexus-surface/30 space-y-2">
                    <Coffee className="w-8 h-8 text-amber-400/40 mx-auto" />
                    <div className="text-nexus-text font-bold">Awaiting QR Scan</div>
                    <div className="text-[11px] text-nexus-text-muted">
                ...
                <div className="space-y-2 pt-2">
                  <div className="text-[11px] font-bold text-nexus-text-muted uppercase">
                    Recent Scans in {currentSlotObj?.label} ({snacksStats.recentScans.length})
                  </div>
                  <div className="max-h-48 overflow-y-auto rounded-xl border border-nexus-border/60 divide-y divide-nexus-border/40 bg-nexus-surface/20">
                    {snacksStats.recentScans.length === 0 ? (
                      <div className="p-4 text-center text-[10px] text-nexus-text-dim">
                      ...
                      snacksStats.recentScans.slice(0, 10).map((r) => (
                        <div
                          key={r.id}
                          className="p-2.5 flex items-center justify-between text-[11px] hover:bg-nexus-surface/50"
                        >
                          <div>
                            <span className="font-bold text-nexus-text">{r.name}</span>
                            <div className="text-[10px] text-nexus-text-dim">
              ...
              <div className="pt-4 border-t border-nexus-border/40 text-[10px] text-nexus-text-dim text-center">
          ...
          {showResetModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
              <div className="w-full max-w-md nexus-glass-glow rounded-2xl p-6 border border-red-500/60 shadow-[0_0_40px_rgba(239,68,68,0.25)] space-y-4">
                <div className="flex items-center justify-between pb-3 border-b border-nexus-border">
                  ...
                  <button
                    ...
                    className="p-1 text-nexus-text-dim hover:text-nexus-text"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>

                <div className="space-y-3 text-xs leading-relaxed text-nexus-text-muted">
                  <p>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-end gap-2 pt-3 border-t border-nexus-border">
                  <button
                    type="button"
                    onClick={() => setShowResetModal(false)}
                    disabled={resetting}
                    className="w-full sm:w-auto px-4 py-2 rounded-xl bg-nexus-surface border border-nexus-border text-nexus-text hover:bg-nexus-surface-elevated text-xs"
                  >
                    CANCEL
                  </button>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            <div className="relative sm:col-span-2">
              ...
              <input
                ...
                className="w-full pl-9 pr-3.5 py-2 rounded-lg bg-nexus-surface border border-nexus-border text-nexus-text focus:outline-none focus:border-nexus-primary"
              />
            </div>
            ...
            <select
              ...
              className="px-3 py-2 rounded-lg bg-nexus-surface border border-nexus-border text-nexus-text focus:outline-none focus:border-nexus-primary"
            >
          ...
          <div className="p-4 rounded-2xl nexus-glass border border-nexus-border flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="flex items-center gap-2.5 text-nexus-text">
              ...
              <div>
                <div className="font-bold text-xs text-nexus-text">EXPORT ABSENTEES REPORT (EXCEL)</div>
                <div className="text-[10px] text-nexus-text-dim">
            ...
            <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
              <button
                ...
                className="px-3.5 py-2 rounded-xl bg-nexus-surface hover:bg-nexus-surface-elevated border border-amber-500/50 text-amber-400 font-bold transition-all flex items-center gap-2 text-xs shadow-sm hover:border-amber-400"
              >
              ...
              <button
                ...
                className="p-2 rounded-xl bg-nexus-surface border border-nexus-border text-nexus-text-muted hover:text-nexus-primary transition-colors"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl nexus-glass border border-nexus-border flex items-center justify-between">
              <div>
                <span className="text-nexus-text-muted text-[10px] block">TOTAL CADETS</span>
                <span className="text-xl font-bold text-nexus-text">{cadets.length}</span>
              </div>
              <Users className="w-6 h-6 text-nexus-primary" />
            </div>
                  ...
                  {attendanceLoading ? (
                    <tr>
                      <td colSpan={7} className="p-8 text-center text-nexus-text-muted">
                        <RefreshCw className="w-5 h-5 animate-spin mx-auto mb-2 text-nexus-primary" />
                    ...
                    filteredCadets.map((cadet) => {
                      ...
                      return (
                        <tr key={cadet.id} className="hover:bg-nexus-surface/50 transition-colors">
                          <td className="p-4 font-bold text-nexus-primary">
                            {cadet.registrationId}
                          </td>
                          <td className="p-4">
                            <div className="font-bold text-nexus-text">{cadet.name}</div>
                            <div className="text-[10px] text-nexus-text-dim">{cadet.email}</div>
                          </td>
                          <td className="p-4 text-nexus-text font-bold">{cadet.registerNumber}</td>
                          <td className="p-4">
