      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-nexus-border">
        <div>
          <h1 className="text-2xl font-bold text-nexus-text tracking-wide flex items-center gap-2">
            <ShieldAlert className="w-6 h-6 text-nexus-primary" />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-nexus-border/60">
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-nexus-text-muted">
                    No audit records recorded yet.
                  </td>
                </tr>
              ) : (
                filtered.map((log) => (
                  <tr key={log.id} className="hover:bg-nexus-surface/50 transition-colors">
                    <td className="p-4 text-nexus-text-dim whitespace-nowrap">
                      {formatDate(log.timestamp)}
                    </td>
                    <td className="p-4">
                      <div className="font-bold text-nexus-text">{log.adminEmail}</div>
                      <div className="text-[10px] text-nexus-text-dim">{log.adminId}</div>
                    </td>
                    <td className="p-4">
                      <span
                    ...
                    <td className="p-4 text-nexus-text uppercase">{log.entity}</td>
                    <td className="p-4 text-nexus-primary font-bold">{log.entityId}</td>
                    <td className="p-4 text-[11px] text-nexus-text-muted font-mono max-w-xs truncate">
