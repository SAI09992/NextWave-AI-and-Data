      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-nexus-border">
        <div>
          <h1 className="text-2xl font-bold text-nexus-text tracking-wide flex items-center gap-2">
            <CreditCard className="w-6 h-6 text-nexus-primary" />
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-nexus-surface border border-nexus-border text-nexus-text hover:text-nexus-primary transition-colors"
        >
          ...
          {(['pending', 'verified', 'rejected', 'all'] as const).map((tab) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg uppercase transition-all ${
                filterTab === tab
                  ? 'bg-nexus-primary text-nexus-bg font-bold shadow-nexus-glow-sm'
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-nexus-border/60">
              {filteredPayments.length === 0 ? (
                <tr>
                  <td colSpan={8} className="p-8 text-center text-nexus-text-muted">
                ...
                filteredPayments.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-nexus-surface/50 transition-colors">
                      <td className="p-4">
                        <div className="font-bold text-nexus-text">{p.participant.name}</div>
                        <div className="text-[10px] text-nexus-text-dim">{p.participant.registrationId}</div>
                      </td>
                      <td className="p-4">
                        <div className="text-nexus-text">{p.participant.registerNumber}</div>
                        <div className="text-[10px] text-nexus-text-dim">{p.participant.department}</div>
                      </td>
                      <td className="p-4">
        ...
        {selectedPayment && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
            <motion.div
              ...
              className="w-full max-w-4xl nexus-glass-glow rounded-3xl p-6 sm:p-8 border border-nexus-primary/50 shadow-2xl space-y-6 my-8"
            >
              {/* Header */}
              <div className="flex items-center justify-between pb-4 border-b border-nexus-border">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-nexus-primary/10 border border-nexus-primary text-nexus-primary">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-nexus-text-muted">
                      PAYMENT REF: {selectedPayment.id} // REG ID: {selectedPayment.participant.registrationId} // UTR: <span className="text-nexus-primary font-mono font-bold">{selectedPayment.utr}</span>
                    </p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="text-nexus-text-dim hover:text-nexus-text p-2"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-6 space-y-4">
                  <div className="p-4 rounded-xl bg-nexus-surface/60 border border-nexus-border space-y-3">
                    <div className="text-nexus-primary font-bold text-xs uppercase tracking-wider">
                      PARTICIPANT DOSSIER
                    </div>
                    ...
                    return (
                      <div className="p-4 rounded-xl bg-nexus-surface/80 border border-yellow-500/40 space-y-3">
                        ...
                        <div className="p-2.5 rounded bg-nexus-bg border border-nexus-border/80 space-y-1.5 text-[11px]">
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-nexus-text-dim font-bold">IMAGE DETECTED UTR:</span>
                ...
                <div className="md:col-span-6 space-y-3">
                  <div className="text-nexus-primary font-bold text-xs uppercase tracking-wider flex items-center justify-between">
                    ...
                    <a
                      ...
                      className="text-[10px] text-nexus-primary hover:underline flex items-center gap-1 font-mono"
                    >
                  ...
                  <div className="p-2 rounded-xl bg-black border border-nexus-border text-center overflow-hidden">
                  ...
                  <div className="p-2 rounded-lg bg-nexus-surface/60 border border-nexus-border space-y-1 text-[10px] font-mono">
                    <span className="text-nexus-text-dim font-bold block">DIRECT BLOB IMAGE URL:</span>
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        readOnly
                        value={selectedPayment.screenshotUrl}
                        className="flex-1 px-2.5 py-1 rounded bg-nexus-bg border border-nexus-border text-nexus-primary font-mono text-[10px] truncate select-all focus:outline-none"
                      />
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="px-2.5 py-1 rounded bg-nexus-primary/20 hover:bg-nexus-primary/30 text-nexus-primary font-bold border border-nexus-primary/40 transition-colors flex items-center gap-1 shrink-0"
                      >
              ...
              <div className="pt-4 border-t border-nexus-border space-y-4">
                <div className="flex items-center gap-3">
                  <span className="text-nexus-text-dim text-[11px]">REJECTION REASON (IF REJECTING):</span>
                  <select
                    value={rejectReason}
                    onChange={(e) => setRejectReason(e.target.value)}
                    className="flex-1 px-3 py-1.5 rounded-lg bg-nexus-surface border border-nexus-border text-nexus-text text-xs"
                  >
                ...
                {rejectReason === 'Other' && (
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-lg bg-nexus-surface border border-nexus-border text-nexus-text text-xs"
                  />
                )}
