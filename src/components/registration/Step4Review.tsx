    ...
    <div className="space-y-6 font-mono text-xs">
      ...
      <div className="p-4 rounded-xl bg-gray-900/50 border border-gray-800 space-y-3">
        <div className="flex items-center justify-between text-yellow-400 font-bold">
          <span className="flex items-center gap-1.5 text-xs">
            <User className="w-4 h-4 text-yellow-400" /> PERSONAL IDENTIFICATION
          </span>
          <button
            type="button"
            onClick={() => onEditStep(1)}
            className="px-2 py-1 rounded bg-gray-950 border border-gray-800 text-gray-400 hover:text-yellow-400 flex items-center gap-1 text-[11px] transition-colors"
          >
      ...
      <div className="p-4 rounded-xl bg-gray-900/50 border border-gray-800 space-y-3">
        <div className="flex items-center justify-between text-red-400 font-bold">
          <span className="flex items-center gap-1.5 text-xs">
            <BookOpen className="w-4 h-4 text-red-400" /> ACADEMIC & CREDIT TRACK
          </span>
          <button
            type="button"
            onClick={() => onEditStep(2)}
            className="px-2 py-1 rounded bg-gray-950 border border-gray-800 text-gray-400 hover:text-red-400 flex items-center gap-1 text-[11px] transition-colors"
          >
            <Edit3 className="w-3 h-3" /> Edit
          </button>
        </div>
      ...
      <div className="p-4 rounded-xl bg-yellow-950/20 border border-yellow-500/30 space-y-3 shadow-[0_0_15px_rgba(255,215,0,0.05)]">
        <div className="flex items-center justify-between text-yellow-400 font-bold">
          <span className="flex items-center gap-1.5 text-xs">
            <FileText className="w-4 h-4 text-yellow-400" /> REGISTRATION FEE BREAKDOWN
          </span>
        ...
        <div className="space-y-2 pt-2 border-t border-yellow-500/20 text-xs">
          ...
          <div className="flex justify-between text-gray-400">
            ...
            <span className="text-orange-400 font-bold">INCLUDED</span>
          </div>
      ...
      <div className="pt-2">
        <label className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors ${isConfirmed ? 'bg-orange-950/30 border-orange-500/50' : 'bg-gray-900/50 border-gray-800 hover:border-gray-600'}`}>
          <input
            type="checkbox"
            checked={isConfirmed}
            onChange={(e) => setIsConfirmed(e.target.checked)}
            className="mt-0.5 flex-shrink-0 appearance-none w-5 h-5 border border-gray-600 rounded bg-gray-950 checked:bg-orange-500 checked:border-orange-500 transition-all relative
