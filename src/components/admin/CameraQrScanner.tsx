...
export function CameraQrScanner({
  ...
  const scannerContainerId = 'nexus-qr-reader';
    ...
    <div className="space-y-4 font-mono text-xs">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-amber-400 font-bold text-sm">
          <Camera className={`w-5 h-5 ${scanning ? 'animate-pulse text-amber-400' : 'text-nexus-text-muted'}`} />
        ...
        <div className="flex items-center gap-2">
          ...
          {cameras.length > 1 && (
            <select
              ...
              disabled={scanning}
              className="px-2.5 py-1 rounded bg-nexus-surface border border-nexus-border text-nexus-text text-[11px] focus:outline-none"
            >
              {cameras.map((cam) => (
      ...
      <div
        ...
        className={`relative rounded-2xl bg-black border-2 transition-all duration-300 overflow-hidden min-h-[260px] flex items-center justify-center ${
          scanning
            ...
            : 'border-nexus-border hover:border-amber-500/60 cursor-pointer group bg-nexus-surface/20'
        }`}
      >
        {/* HTML5 QR Container */}
        ...
        {!scanning && (
          <div className="p-8 text-center space-y-4">
            ...
            <div>
              <span className="text-nexus-text font-bold block text-sm">
                Single-Shot QR Scanner (Click to Scan)
              </span>
              <span className="text-nexus-text-muted text-[11px] block mt-1">
        ...
        {error && (
          <div className="absolute inset-0 bg-black/90 p-6 flex flex-col items-center justify-center text-center space-y-2">
            ...
            <span className="text-[10px] text-nexus-text-dim max-w-xs">
