...
export default function AdminSidebar({ isOpen, setIsOpen }: { isOpen?: boolean, setIsOpen?: (val: boolean) => void }) {
  ...
  return (
    <>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition-all ${
                  isActive
                    ? 'bg-nexus-primary/15 text-nexus-primary font-bold border border-nexus-primary/40 shadow-nexus-glow-sm'
      ...
      {/* Footer / Logout */}
      <div className="pt-4 border-t border-nexus-border/80 space-y-2">
        <Link
          href="/"
          className="flex items-center gap-2 px-3 py-2 rounded-lg text-nexus-text-dim hover:text-nexus-primary text-[11px] transition-colors"
        >
