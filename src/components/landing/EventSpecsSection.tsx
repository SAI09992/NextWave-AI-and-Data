...
export default function EventSpecsSection() {
  ...
  return (
    <section id="specs" className="py-16 sm:py-24 relative bg-transparent border-t border-nexus-border/60">
          ...
          {specCards.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -3 }}
                className="p-5 rounded-2xl nexus-glass border border-nexus-border hover:border-nexus-primary/40 transition-all duration-300 flex items-start gap-4 font-mono"
              >
                <div className="p-3 rounded-xl bg-nexus-surface border border-nexus-border text-nexus-primary shrink-0 shadow-nexus-glow-sm">
                  <Icon className="w-5 h-5" />
                </div>
                <div className="space-y-1 min-w-0 flex-1">
                  <div className="text-[10px] text-nexus-text-dim uppercase tracking-wider font-bold">
                    {item.label}
                  </div>
                  <div className="text-xs sm:text-sm font-bold text-nexus-text">
                    {item.value}
                  </div>
                  <div className="text-[11px] text-nexus-primary">
                    {item.highlight}
                  </div>
                </div>
              </motion.div>
