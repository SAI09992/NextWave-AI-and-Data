...
const infoItems = [
  ...
  },
  {
    ...
    desc: 'Cadets must adhere to responsible nexus disclosure and ethical range rules. Malicious activities outside lab scopes will lead to disqualification.',
  },
];

export default function ImportantInfoSection() {
  return (
    <section id="guidelines" className="py-16 sm:py-24 relative border-t border-nexus-border/60">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14 space-y-3">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-yellow-950/60 border border-nexus-primary/40 text-nexus-primary text-xs font-mono">
            ...
            <span>// 11. IMPORTANT DIRECTIVES & GUIDELINES</span>
          </div>
          ...
          {infoItems.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                className="p-5 rounded-2xl nexus-glass border border-nexus-border hover:border-nexus-primary/40 transition-all font-mono space-y-2.5"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-xl bg-nexus-surface border border-nexus-border text-nexus-primary">
                    <Icon className="w-4 h-4" />
                  </div>
                  <h3 className="text-sm font-bold text-nexus-text">{item.title}</h3>
                </div>
                <p className="text-xs text-nexus-text-muted font-sans leading-relaxed">
                  {item.desc}
                </p>
              </motion.div>
            );
