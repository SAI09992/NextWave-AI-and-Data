...
export default function CinematicPreloader({ onComplete }: PreloaderProps) {
  ...
  const [logText, setLogText] = useState('> INITIALIZING NEXTWAVE AI FRAMEWORK...');
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (current < 25) setLogText('> INITIALIZING NEXTWAVE AI FRAMEWORK...');
      else if (current < 50) setLogText('> DEPLOYING NEURAL DATA PIPELINE...');
      else if (current < 75) setLogText('> CALIBRATING AI MODEL ARCHITECTURE...');
      else if (current < 99) setLogText('> ESTABLISHING SECURE DATA FEED...');
      else setLogText('> SYSTEM SYNCHRONIZED // ENGAGING REVEAL');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#030712] overflow-hidden font-mono text-nexus-primary selection:bg-none">
      ...
      <AnimatePresence>
        {phase === 1 && (
          ...
          >
            ...
            <div className="relative flex items-center justify-center w-64 h-64 rounded-full border border-nexus-primary/20 bg-nexus-bg/20 backdrop-blur-sm shadow-[0_0_60px_rgba(16,185,129,0.1)]">
              {/* Rotating outer dash ring */}
              <motion.div 
                animate={{ rotate: 360 }} 
                transition={{ duration: 15, repeat: Infinity, ease: 'linear' }}
                className="absolute inset-[-15px] rounded-full border border-dashed border-nexus-primary/40"
              />
              <motion.div 
                animate={{ rotate: -360 }} 
                transition={{ duration: 25, repeat: Infinity, ease: 'linear' }}
                className="absolute inset-[-25px] rounded-full border border-dotted border-nexus-primary/20"
              />
              
              {/* Progress Text */}
              <div className="text-center">
                <div className="text-6xl font-black tracking-tighter text-nexus-primary drop-shadow-[0_0_15px_rgba(16,185,129,0.8)]">
                  {progress.toString().padStart(2, '0')}<span className="text-3xl text-nexus-primary/60">%</span>
                </div>
                <div className="text-[10px] mt-2 text-nexus-primary/70 tracking-widest font-bold">
                  SYS_BOOT_SEQ
                </div>
              </div>
            </div>
            ...
            <div className="absolute bottom-16 w-full text-center">
              <div className="text-xs text-nexus-text-muted tracking-widest uppercase opacity-80 h-4">
                {logText}
              </div>
            </div>
          </motion.div>
        ...
        {phase === 3 && (
          ...
          >
            ...
            <motion.div
              ...
              className="text-nexus-primary/90 text-sm md:text-base tracking-[0.4em] mb-4 font-bold"
            >
              NEXTWAVE PRESENTS
            </motion.div>
            ...
            <motion.div
              ...
              className="mt-8 flex flex-wrap justify-center gap-3 md:gap-6 text-[10px] md:text-sm font-mono text-nexus-text-muted tracking-[0.2em]"
            >
              <span className="text-white">ARTIFICIAL INTELLIGENCE</span>
              <span className="text-nexus-primary/40">//</span>
            ...
            <motion.div 
              ...
              className="absolute bottom-0 w-full h-[3px] bg-nexus-primary shadow-[0_0_30px_#10b981]"
            />
          </motion.div>
        )}
      </AnimatePresence>
