...
export default function PaymentScannerAnimation({ onComplete, ocrDetails }: Props) {
  ...
  return (
    <div className="w-full max-w-xl mx-auto nexus-glass-glow rounded-2xl p-6 sm:p-8 border border-nexus-primary/40 text-center">
      {/* Scanner Radar Arc */}
      <div className="relative w-20 h-20 mx-auto mb-6 flex items-center justify-center">
        <div className="absolute inset-0 rounded-full border-2 border-nexus-primary/30 animate-ping" />
        <div className="absolute inset-0 rounded-full border-2 border-dashed border-nexus-primary animate-radar-sweep" />
        <div className="relative z-10 w-12 h-12 rounded-full bg-nexus-surface flex items-center justify-center text-nexus-primary">
          <Cpu className="w-6 h-6 animate-pulse" />
        </div>
      </div>

      <h3 className="text-xl font-bold tracking-wider text-nexus-primary font-mono mb-2">
        SOC PAYMENT INTELLIGENCE
      </h3>
      <p className="text-xs text-nexus-text-muted mb-8 font-mono">
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-nexus-text-muted truncate">{step.desc}</div>
              </div>
            </motion.div>
          );
        })}
      ...
      {ocrDetails?.extractedUtr && (
        <motion.div
          ...
          className="mt-6 p-3 rounded-lg bg-nexus-bg-elevated border border-nexus-border text-left font-mono text-xs text-nexus-text-muted"
        >
          ...
          {ocrDetails.extractedAmount && (
            <div>Amount: <span className="text-nexus-text">₹{ocrDetails.extractedAmount}</span></div>
          )}
        </motion.div>
      )}
    </div>
