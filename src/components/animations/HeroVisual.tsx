...
export default function HeroVisual() {
  return (
    <div className="relative w-full max-w-[500px] aspect-square mx-auto flex items-center justify-center select-none">
      {/* Outer Cyan nexus Flare */}
      <div className="absolute inset-0 bg-gradient-to-tr from-nexus-primary/20 via-transparent to-nexus-secondary/20 rounded-full blur-3xl animate-pulse-glow" />

      {/* Layer 1: Outer Rotating Angle Coordinate Reticle */}
      <motion.div
        animate={{ rotate: 360 }}
        transition={{ repeat: Infinity, duration: 40, ease: 'linear' }}
        className="absolute inset-2 rounded-full border border-dashed border-nexus-primary/25 flex items-center justify-between px-2"
      >
        <span className="text-[8px] font-mono text-nexus-primary/60 bg-nexus-bg px-1">000°</span>
        <span className="text-[8px] font-mono text-nexus-primary/60 bg-nexus-bg px-1">180°</span>
      </motion.div>
      ...
      <motion.div
        ...
        className="absolute inset-8 rounded-full border-2 border-dashed border-nexus-secondary/30"
      />
      ...
      <motion.div
        ...
        className="absolute inset-16 rounded-full border border-nexus-primary/40 shadow-nexus-glow-sm"
      />

      {/* Layer 4: Central Tactical nexus Shield SVG */}
      ...
      <motion.div
        ...
        className="absolute top-4 left-0 sm:-left-6 nexus-glass px-3.5 py-1.5 rounded-xl text-[11px] font-mono text-nexus-primary flex items-center gap-2 shadow-nexus-card border border-nexus-border/80"
      >
        <span className="w-2 h-2 rounded-full bg-nexus-primary animate-ping" />
